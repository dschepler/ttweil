...
\begin{document}
	...
	In the examples of a presheaf or sheaf topos, we have $\mathcal{F}(U) \simeq \Hom(h_U, \mathcal{F})$, where $h_U(V) = \begin{cases} \{ * \}, & V \subseteq U, \\ \emptyset, & V \not\subseteq U. \end{cases}$  In a more general topos, it will turn out to be useful to continue to think of a morphism $x \in \Hom(U, X)$ as a generalized element of $X$.  We will show that it will be easy to construct many functions on generalized elements of the form below.

	\begin{definition}
		Let $\calT$ be a topos, and let $A_1, \ldots, A_n, B$ be objects of $\calT$.  Then a (semantic) term of type $B$ with free variables of types $A_1, \ldots, A_n$ is a natural transformation $\prod_{i=1}^n \Hom({-}, A_i) \to \Hom({-}, B)$.  We will often informally describe such a term as an expression with named free variables $a_1 : A_1, \ldots, a_n : A_n$.
	\end{definition}
	Note that by Yoneda's lemma, this is equivalent to a morphism $\prod_{i=1}^n A_i \to B$.  However, we will usually use the original form of the definition, in order to continue thinking of it as an expression where we can plug in generalized elements of $A_i$ and get a generalized element of $B$.
	...
	\begin{definition}
		If $\tau$ is a term of type $X$ with free variables $a_1 : A_1, \ldots, a_n : A_n$, and $f \in \Hom(X, Y)$, then $f(\tau)$ is the term of type $Y$ with the same set of free variables defined by $f(\tau)(\alpha_1, \ldots, \alpha_n) = f \circ \tau(\alpha_1, \ldots, \alpha_n)$.
	\end{definition}
	
	\begin{definition}
		$\top$ will be the term of type $\Omega$ (with any number of desired free variables) which at object $U$ of the topos acts as the constant function with value in $\Hom(U, \Omega)$ corresponding to the subobject $\id_U : U \to U$.
	\end{definition}
	...
	\begin{definition}
		Let $\alpha_i \in \Hom(U, A_i)$ for $1 \le i \le n$, and let $\varphi$ be a semantic term of type $\Omega$ with free variables $a_1 : A_1, \ldots, a_n : A_n$.  Then $U \models \varphi(a_1 := \alpha_1, \ldots, a_n := \alpha_n)$ if and only if $\varphi_U(\alpha_1, \ldots, \alpha_n) = \top_U$.
	\end{definition}
	
	\begin{proposition}
		A semantic term of type $\Omega$ with free variables $a_1 : A_1, \ldots, a_n : A_n$ is uniquely determined by the set of objects $U$ and tuples $\alpha_i \in \Hom(U, A_i)$ such that $U \models \varphi(a_1 := \alpha_1, \ldots, a_n := \alpha_n)$.
	...
	In what follows, we will give conditions for each term of type $\Omega$ indicating when that term is satisfied for some tuple of generalized elements.  These conditions together will form what is known as the Kripke-Joyal semantics.  We will start off with a couple general properties.
	
	\begin{proposition}
		Suppose $\tau$ is a term of type $\Omega$ with free variables $a_1 : A_1, \ldots, a_n : A_n$; $U$ is an object; $\alpha_i \in \Hom(U, A_i)$; and $\pi : V \to U$ is a morphism.
		\begin{enumerate}
			\item If $U \models \varphi[a_1 := \alpha_1, \ldots, a_n := \alpha_n]$, then $V \models \varphi[a_1 := \alpha_1 \circ \pi, \ldots, a_n := \alpha_n \circ \pi]$.
			\item If $\pi$ is an epimorphism, then the converse holds: if $V \models \varphi[a_1 := \alpha_1 \circ \pi, \ldots, a_n := \alpha_n \circ \pi]$, then $U \models \varphi[a_1 := \alpha_1, \ldots, a_n := \alpha_n]$.
		\end{enumerate}
		
		\begin{proof}
			\begin{enumerate}
				\item From the naturality of $\varphi$, we see $\varphi(\alpha_1 \circ \pi, \ldots, \alpha_n \circ \pi) = \varphi(\alpha_1, \ldots, \alpha_n) \circ \pi = \top_U \circ \pi = \top_V$.
				\item From the hypothesis, $\varphi(\alpha_1, \ldots, \alpha_n) \circ \pi = \top_V = \top_U \circ \pi$.  Since $\pi$ is an epimorphism, that implies $\varphi(\alpha_1, \ldots, \alpha_n) = \top_U$.
			\end{enumerate}
		\end{proof}
	\end{proposition}
	
	\begin{proposition}
		For all objects $U$ and generalized elements $\alpha_i \in \Hom(U, A_i)$, $U \models \top[a_1 := \alpha_1, \ldots, a_n := \alpha_n]$.
	...
	\begin{definition}
		Suppose $\sigma$ and $\tau$ are terms of type $X$ with the same set of free variables $a_i : A_i$.  Then $\sigma = \tau$ is a term of type $\Omega$ with the same set of free variables corresponding to the natural transformation $\prod_{i=1}^n \Hom({-}, A_i) \to \Sub$ sending $(\alpha_i)$ to the equalizer of $\sigma(\alpha_1, \ldots, \alpha_n)$ and $\tau(\alpha_1, \ldots, \alpha_n)$.
	\end{definition}
	
	\begin{proposition}
		For generalized elements $\alpha_i \in \Hom(U, A_i)$, $U \models (\sigma = \tau)[a_1 := \alpha_1, \ldots, a_n := \alpha_n]$ if and only if $\sigma(\alpha_1, \ldots, \alpha_n) = \tau(\alpha_1, \ldots, \alpha_n)$ in $\Hom(U, X)$.
	\end{proposition}
	
	\subsection{Membership Propositions}
	
	\begin{definition}
		Suppose $\sigma$ is a term of type $X$ and $\Sigma$ is a term of type $PX$ with the same set of free variables $a_i : A_i$.  Then $\sigma \in \Sigma$ is the term of type $\Omega$ with the same set of free variables corresponding to the natural transformation constructed as follows: from $x := \sigma(a_1, \ldots, a_n) \in \Hom(U, X)$ and $\Sigma(a_1, \ldots, a_n) \in \Hom(U, PX)$, we convert the latter to the corresponding subobject $S \hookrightarrow U \times X$, and then take the pullback of $S$ by the graph of $x$, which is the morphism $\Gamma_x := (\id_U, x) : X \to U \times X$.
	\end{definition}
	\begin{proposition}
		$U \models (\sigma \in \Sigma)[a_1 := \alpha_1, \ldots, a_n := \alpha_n]$ if and only if the graph of $\sigma(a_1, \ldots, a_n)$ factors through the subobject of $U\times X$ corresponding to $\Sigma(a_1, \ldots, a_n)$.
	\end{proposition}
	
	\subsection{Conjunction}
	
	\begin{definition}
		Suppose $\varphi$ and $\psi$ are terms of type $\Omega$ with the same set of free variables $a_i : A_i$.  Then $\varphi \wedge \psi$ is the term of type $\Omega$ corresponding to the natural transformation constructed as follows: take the subobjects $S$ and $T$ of $U$ corresponding to $\varphi(\alpha_1, \ldots, \alpha_n)$ and $\psi(\alpha_1, \ldots, \alpha_n)$; take the intersection subobject $S \times_U T$; and convert this back to a morphism $U \to \Omega$.
	\end{definition}
	\begin{proposition}
		$U \models (\varphi \wedge \psi)[a_1 := \alpha_1, \ldots, a_n := \alpha_n]$ if and only if $U \models \varphi[a_1 := \alpha_1, \ldots, a_n := \alpha_n]$ and $U \models \psi[a_1 := \alpha_1, \ldots, a_n := \alpha_n]$.
		\begin{proof}
	...
	\begin{definition}
		Suppose $\varphi$ and $\psi$ are terms of type $\Omega$ with the same set of free variables $a_i : A_i$.  Then $\varphi \leftrightarrow \psi$ is another name for $\varphi = \psi$.
	\end{definition}
	\begin{proposition}
		$U \models (\varphi \leftrightarrow \psi)[a_1 := \alpha_1, \ldots, a_n := \alpha_n]$ if and only if for each object $V$ and morphism $\pi : V \to U$, $V \models \varphi[a_1 := \alpha_1 \circ \pi, \ldots, a_n := \alpha_n \circ \pi]$ if and only if $V \models \psi[a_1 := \alpha_1 \circ \pi, a_n := \alpha_n \circ \pi]$.
		\begin{proof}
			($\Rightarrow$): From the hypothesis, we know that $\varphi(\alpha_1, \ldots, \alpha_n) = \psi(\alpha_1, \ldots, \alpha_n)$.  Also, since $\varphi$ and $\psi$ are natural transformations, we have $\varphi(\alpha_1 \circ \pi, \ldots, \alpha_n \circ \pi) = \varphi(\alpha_1, \ldots, \alpha_n) \circ \pi$, and similarly for $\psi$.
			
			($\Leftarrow$): If we apply the hypothesis with $V$ being the subobject of $U$ corresponding to $\varphi(\alpha_1, \ldots, \alpha_n)$, we get that $\varphi(\alpha_1, \ldots, \alpha_n) \le \psi(\alpha_1, \ldots, \alpha_n)$.  Similarly, if $V$ is the subobject corresponding to $\psi(\alpha_1, \ldots, \alpha_n)$, we get $\psi(\alpha_1, \ldots, \alpha_n) \le \varphi(\alpha_1, \ldots, \alpha_n)$.
		\end{proof}
	\end{proposition}
	...
	\begin{definition}
		Suppose $\varphi$ and $\psi$ are terms of type $\Omega$ with the same set of free variables $a_i : A_i$.  Then $\varphi \rightarrow \psi$ is the term of type $\Omega$ given by $(\varphi \land \psi) \leftrightarrow \varphi$.
	\end{definition}
	\begin{proposition}
		$U \models (\varphi \rightarrow \psi)[a_1 := \alpha_1, \ldots, a_n := \alpha_n]$ if and only if for every object $V$ and morphism $\pi : V \to U$ such that $V \models \varphi[a_1 := \alpha_1 \circ \pi, \ldots, a_n := \alpha_n \circ \pi]$, we also have $V \models \psi[a_1 := \alpha_1 \circ \pi, \ldots, \alpha_n \circ \pi]$.
		\begin{proof}
	...
	\begin{definition}
		Suppose $\varphi$ is a term of type $\Omega$ with free variables $x:X, a_1:A_1, \ldots, a_n:A_n$.  Then $\{ x : X \mid \varphi \}$ is the term of type $PX$ with free variables $a_1:A_1, \ldots, a_n:A_n$ constructed as follows: from $\alpha_i \in \Hom(U, A_i)$, we build a morphism of functors $\Hom({-}, U) \times \Hom({-}, X) \to \Hom({-}, \Omega)$ which sends $\pi \in \Hom(V, U)$ and $x \in \Hom(V, X)$ to $\varphi(x, \alpha_1 \circ \pi, \ldots, \alpha_n \circ \pi)$.  This is equivalent to a morphism $U \times X \to \Omega$, which corresponds to a subobject of $U \times X$, which in turn corresponds to a morphism in $\Hom(U, PX)$.
	\end{definition}
	
	\begin{definition}
		Suppose $\tau$ is a term of type $X$ with free variables $a_1:A_1, \ldots, a_n:A_n$.  Then $\{ \tau \}$ is the term of type $PX$ with the same set of free variables corresponding to the natural transformation $\prod_{i=1}^n \Hom({-}, A_i) \to \Sub(A_1 \times \cdots \times A_n \times X)$ mapping $(\alpha_1, \ldots, \alpha_n)$ to the subobject $(\pi_1, \ldots, \pi_n, \tau(\alpha_1, \ldots, \alpha_n)) : A_1 \times \cdots \times A_n \to A_1 \times \cdots \times A_n \times X$.
	...
	\begin{definition}
		Suppose $\varphi$ is a term of type $\Omega$ with free variables $x:X, a_1:A_1, \ldots, a_n:A_n$.  Then $\forall x:X, \varphi$ is the term of type $\Omega$ with free variables $a_1:A_1, \ldots, a_n:A_n$ given by $\{ x:X \mid \varphi \} = \{ x:X \mid \top \}$.
	\end{definition}
	\begin{proposition}
		$U \models (\forall x:X, \varphi)[a_1 := \alpha_1, \ldots, a_n := \alpha_n]$ if and only if for each object $V$, morphism $\pi : V \to U$, and generalized element $x \in \Hom(V, X)$, we have $V \models \varphi[x := x, a_1 := \alpha_1 \circ \pi, \ldots, a_n := \alpha_n \circ \pi]$.
		\begin{proof}
			We have $U \models (\forall x:X, \varphi)(a_1 := \alpha_1, \ldots, a_n := \alpha_n)$ if and only if the subobject of $U\times X$ is the full subobject $\id_{U\times X}$, which is equivalent to the morphism $U\times X \to \Omega$ being equal to $\top_U \circ \pi_1 : U\times X \to U \to \Omega$.  That, in turn, is equivalent to the morphism of functors $\Hom({-}, U) \times \Hom({-}, X) \to \Hom({-}, \Omega)$ being equal to the natural transformation which for each object $V$, morphism $\pi \in \Hom(V, U)$, and morphism $x \in \Hom(V, X)$ maps $(\pi, x) \mapsto \top_V$.
		\end{proof}
	\end{proposition}
	...
	\begin{definition}
		$\bot$ will be the term of type $\Omega$ (with any number of desired free variables) defined by $\forall p : \Omega, p$ (where $p$ is a fresh free variable not in the desired set of free variables).
	\end{definition}
	\begin{proposition}
		$U \models \bot[a_1 := \alpha_1, \ldots, a_n := \alpha_n]$ if and only if for every object $V$ with a morphism to $U$, $V$ is an initial object of the topos.
		\begin{proof}
			...
			On the other hand, if we have any two morphisms $x_1, x_2 : U \to X$, let $p : U \to \Omega$ correspond to the equalizer of $x_1$ and $x_2$.  Then by the assumption, $U \models p[p := p, a_1 := \alpha_1, \ldots, a_n := \alpha_n]$, implying that $p = \top_U$; thus, the equalizer of $x_1$ and $x_2$ is the full subobject of $U$, so $x_1 = x_2$.
			
			($\Leftarrow$) Let $\pi : V \to U$ be any morphism and $p : V \to \Omega$ any generalized element.  Then since $V$ is initial, $p = \top_V$, so $V \models p[p:=p, a_1:=\alpha_1\circ\pi, \ldots, a_n:=\alpha_n\circ\pi]$.
		\end{proof}
	\end{proposition}
	...
	\begin{corollary}
		$U \models \lnot \varphi[a_1 := \alpha_1, \ldots, a_n := \alpha_n]$ if and only if for each $\pi : V \to U$ such that $V \models \varphi[a_1 := \pi^* \alpha_1, \ldots, a_n := \pi^* \alpha_n]$, $V$ is initial.
	\end{corollary}
	
	\subsection{Disjunction}
	
	\begin{definition}
		Let $\varphi$ and $\psi$ be two terms of type $\Omega$ with the same set of free variables $a_1:A_1, \ldots, a_n:A_n$.  Then $\varphi \lor \psi$ will be the term of type $\Omega$ with the same set of free variables defined by $\forall r : \Omega, (\varphi \rightarrow r) \rightarrow ((\psi \rightarrow r) \rightarrow r)$ (where $r$ is a fresh free variable not in the set $a_1, \ldots, a_n$).
	\end{definition}
	\begin{proposition}
		$U \models (\varphi \lor \psi)[a_1:=\alpha_1, \ldots, a_n:=\alpha_n]$ if and only if for each morphism $\pi : V \to U$, there exist $\sigma_1 : W_1 \to V$ and $\sigma_2 : W_2 \to V$ such that $W_1 \models \varphi[\alpha_1\circ \pi\circ \sigma_1, \ldots, \alpha_n\circ \pi\circ \sigma_1]$, $W_2 \models \psi[\alpha_1 \circ \pi\circ \sigma_2, \ldots, \alpha_n \circ \pi \circ \sigma_2]$, and $\sigma_1, \sigma_2$ are an epimorphic family (i.e. whenever $x_1, x_2 : V \to X$ satisfy $x_1 \circ \sigma_1 = x_2 \circ \sigma_1$ and $x_1 \circ \sigma_2 = x_2 \circ \sigma_2$, then $x_1 = x_2$).
		\begin{proof}
			($\Rightarrow$): Since $\models$ is stable under pullbacks, it suffices to show the case $V=U$, $\pi = \id_U$.  Let $\sigma_1 : W_1 \to U$ be the subobject of $U$ corresponding to $\varphi(\alpha_1, \ldots, \alpha_n)$, and $\sigma_2 : W_2 \to U$ the subobject of $U$ corresponding to $\psi(\alpha_1, \ldots, \alpha_n)$.  It is easy to see that $W_1\models\varphi[\alpha_1\circ\sigma_1, \ldots, \alpha_n\circ\sigma_1]$ and $W_2\models\psi[\alpha_1\circ\sigma_2, \ldots, \alpha_n\circ\sigma_2]$.  It remains to check that $\sigma_1, \sigma_2$ are an epimorphic family.  To see this, suppose $x_1, x_2 : U \to X$ are as in the specification, and consider $r : U \to \Omega$ corresponding to the equalizer of $x_1$ and $x_2$.  Then we can check that $\varphi \rightarrow r$ evaluates to $\top$ since any $W'\to U$ such that $W' \models \varphi$ factors through $\sigma_1$; similarly, $\psi \rightarrow r$ evaluates to $\top$.  Therefore, by the assumption, we must have $r = \top_U$, so $x_1 = x_2$.
			
			($\Leftarrow$): For any $\pi:V\to U$ and $r\in \Hom(V, \Omega)$, choose $W_1, \sigma_1, W_2, \sigma_2$ as in the hypothesis, and suppose $V \models (\phi \rightarrow r)[r:=r, a_1:=\alpha_1\circ\pi, \ldots, a_n:=\alpha_n\circ\pi]$ and $V\models (\psi\rightarrow r)[r:=r, a_1:=\alpha_1\circ \pi, \ldots, a_n:=\alpha_n\circ \pi]$.  Then since $W_1 \models (\phi \rightarrow r)[r:=r\circ\sigma_1, a_1:=\alpha_1\circ\pi\circ\sigma_1, \ldots, a_n:=\alpha_n\circ\pi\circ\sigma_n]$, we must have $r \circ \sigma_1 = \top_{W_1} = \top_V \circ \sigma_1$.  Similarly, $r \circ \sigma_2 = \top_V \circ \sigma_2$.  Therefore, since we are assuming $\sigma_1, \sigma_2$ are an epimorphic family, we have $r = \top_V$, so $V \models r[r:=r, a_1:=\alpha_1\circ\pi, \ldots, a_n:=\alpha_n\circ\pi]$.
		\end{proof}
	\end{proposition}
	...
	\begin{definition}
		Let $\varphi$ be a term of type $\Omega$ with free variables $x:X, a_1:A_1, \ldots, a_n:A_n$.  Then $\exists x:X, \varphi$ will be the term of type $\Omega$ with free variables $a_1:A_1, \ldots, a_n:A_n$ defined by $\forall q:\Omega, (\forall x:X, \varphi \rightarrow q) \rightarrow q$ (where $q$ is a fresh free variable not in the set $x, a_1, \ldots, a_n$).
	\end{definition}
	\begin{proposition}
		$U \models (\exists x:X, \varphi)[a_1:=\alpha_1, \ldots, a_n:=\alpha_n]$ if and only if for each morphism $\pi : V \to U$, there exists an epimorphism $\sigma : W \to V$ and a generalized element $x \in \Hom(W, X)$ such that $W \models \varphi[x:=x, a_1:=\alpha_1\circ\pi\circ\sigma, \ldots, a_n:=\alpha_n\circ\pi\circ\sigma]$.
		\begin{proof}
			($\Rightarrow$): Since $\models$ is stable under pullback, it suffices to show the case where $V = U$ and $\pi = \id_U$.  Now let us consider $W_0 := U\times X$ and $x_0 : W_0 \to X$, $\sigma_0 : W_0 \to U$ the projections.  Then $\varphi(x_0, \alpha_1\circ \sigma_0, \ldots, \alpha_n \circ \sigma_0) \in \Hom(W_0, \Omega)$ corresponds to a subobject $i_W : W \hookrightarrow W_0$.  By construction, we have $W \models \varphi[x:=x_0\circ \sigma_0, a_1:=\alpha_1\circ \sigma_0\circ i_W, \ldots, a_n:=\alpha_n\circ \sigma_0\circ i_W]$.  We claim that $\sigma_0 \circ i_W$ is an epimorphism.
			
			To see this, suppose we have two morphisms $f, g : U \to Y$ such that $f\circ \sigma_0 \circ i_W = g\circ \sigma_0 \circ i_W$, and let $q \in \Hom(U, \Omega)$ correspond to the equalizer of $f$ and $g$.  Then for any $\pi' : V' \to U$ and $x' \in \Hom(V', X)$ such that $V' \models \varphi[x:=x', a_1:=\alpha_1\circ \pi', \ldots, a_n:=\alpha_n\circ \pi']$, we must have $(\pi', x') \in \Hom(V', W_0)$ factors through $W$.  As a result, $f \circ \pi' = g \circ \pi'$, so $V' \models q[q:=q, \ldots]$.  This shows that $U \models (\forall x:X, \varphi \rightarrow q)[q:=q, a_1:=\alpha_1, \ldots, a_n:=\alpha_n]$.  We may conclude that $U \models q[q:=q, \ldots]$, so $f = g$.
			
			($\Leftarrow$): Suppose we have a morphism $\pi\in \Hom(V, U)$ and a generalized element $q \in \Hom(V, \Omega)$, such that $V \models (\forall x:X, \varphi \rightarrow q)[q:=q, a_1:=\alpha_1\circ\pi, \ldots, a_n:=\alpha_n\circ\pi]$.  Choose an epimorphism $\sigma : W \to V$ and generalized element $x\in \Hom(W, X)$ as in the hypothesis.  Then since $W\models\varphi[x:=x, q:=q, a_1:=\alpha_1, \ldots, a_n:=\alpha_n]$, we also have $W\models q[x:=x, q:=q\circ\sigma, a_1:=\alpha_1\circ\pi\circ\sigma, \ldots, a_n:=\alpha_n\circ\pi\circ\sigma]$, i.e. $q\circ \sigma = \top_W = \top_V\circ\sigma$.  Since $\sigma$ is an epimorphism, it follows that $V\models q[q:=q, a_1:=\alpha_1\circ\pi, \ldots, a_n:=\alpha_n\circ\pi]$.
		\end{proof}
	\end{proposition}
	...
	\begin{definition}
		Let $\varphi$ be a term of type $\Omega$ with free variables $x:X, a_1:A_1, \ldots, a_n:A_n$.  Then $\exists! x:X, \varphi$ will be the term of type $\Omega$ with free variables $a_1:A_1, \ldots, a_n:A_n$ defined as $\chi_{\{ \cdot \}}(\{ x:X \mid \varphi \})$ where $\chi_{\{ \cdot \}}$ is the morphism $PX \to \Omega$ corresponding to the singleton subobject $\{ \cdot \} : X \hookrightarrow PX$.
	\end{definition}
	\begin{proposition}
		$U \models (\exists! x:X, \varphi)[a_1:=\alpha_1, \ldots, a_n:=\alpha_n]$ if and only if for every morphism $\pi : V \to U$ there exists a unique $x \in \Hom(V, X)$ such that $V \models \varphi[x:=x, a_1:=\alpha_1\circ\pi, \ldots, a_n:=\alpha_n\circ\pi]$.
		
		\begin{proof}
			($\Rightarrow$): From the hypothesis, we see that the morphism $V \to PX$ induced by $\varphi$ and $\alpha_1 \circ \pi, \ldots, \alpha_n \circ \pi$ factors uniquely through $X$, and the unique factor map $V \to X$ gives $x$.
			
			($\Leftarrow$): Let $x_0 \in \Hom(U, X)$ be the unique such $x$ for the case $V = U$ and $\pi = \id_U$.  Then from the hypotheses, we can conclude that $\{ x:X \mid \varphi \}(\alpha_1, \ldots, \alpha_n) \in \Hom(U, PX)$ is equal to $\{ \cdot \} \circ x_0$, so composing with $\chi_{\{ \cdot \}}$ will give $\top_U$.
		\end{proof}
	\end{proposition}
	...
	\begin{proposition}
		\begin{align*}
			1 \models & \forall a_1:A_1, \ldots, \forall a_n:A_n, \\
			& (\exists! x:X, \varphi) \leftrightarrow \\
			& [(\exists x:X, \varphi) \wedge \\
			& (\forall x_1:X, \forall x_2:X, \varphi[x:=x_1] \land \varphi[x:=x_2] \rightarrow x_1 = x_2)].
		\end{align*}
		Here $\varphi[x:=x_1]$ is the term with free variables $x_1:X, x_2:X, a_1:A_1, \ldots, a_n:A_n$ gotten by composing $\varphi$ with $\Hom({-}, X) \times \Hom({-}, X) \times \prod_{i=1}^n \Hom({-}, A_i) \to \Hom({-}, X) \times \prod_{i=1}^n \Hom({-}, A_i), (x_1, x_2, \alpha_1, \ldots, \alpha_n) \mapsto (x_1, \alpha_1, \ldots, \alpha_n)$, and similarly for $\varphi[x:=x_2]$.
		
		\begin{proof}
			Here we will focus on the interesting part, which is the $\leftarrow$ part.  Thus, suppose we have $U \models (\exists x:X, \varphi)[a_1:=\alpha_1, \ldots, a_n:=\alpha_n]$ and $U \models (\forall x_1:X, \forall x_2:X, \phi[x:=x_1] \land \phi[x:=x_2] \rightarrow x_1 = x_2)[a_1:=\alpha_1, \ldots, a_n:=\alpha_n]$.  Then from the first, we can find an epimorphism $\pi : V \to U$ and $x \in \Hom(V, X)$ such that $V \models \phi[x:=x, a_1:=\alpha_1\circ\pi, \ldots, a_n:=\alpha_n\circ\pi]$.  Now from the second, we see that $V \models (\forall x_2:X, \phi[x:=x_2] \rightarrow x = x_2)[x:=x, a_1:=\alpha_1 \circ \pi, \ldots, a_n:=\alpha_n \circ \pi]$.  From this, we can conclude that $\{ x:X \mid \varphi \}(\alpha_1\circ\pi, \ldots, \alpha_n\circ\pi) = \{ x \}$ as elements of $\Hom(V, PX)$.  It follows that $V \models (\exists! x:X, \varphi)[a_1:=\alpha_1\circ\pi, \ldots, a_n:=\alpha_n\circ\pi]$.  Since $\pi$ was an epimorphism, that implies that $U \models (\exists! x:X, \varphi)[a_1:=\alpha_1\circ \pi, \ldots, a_n:=\alpha_n\circ\pi]$.
		\end{proof}
	\end{proposition}
	...
	\begin{definition}
		Let $X$ be an object a topos, and let $S$ be a term of type $PX$ with no free variables.  Then $\Sigma S$ will be the subobject of $X$ corresponding to $S_1$ via the bijection $\Hom(1, PX) \simeq \Sub(1 \times X) \simeq \Sub(X)$.  In accordance with the next definition, we may refer to the inclusion map as $\drop$.
	...
	\begin{definition}
		Suppose $\tau$ is a term of type $X$ with free variables $a_1:A_1, \ldots, a_n:A_n$, $\varphi$ is a term of type $\Omega$ with the same free variables, and suppose that we have $1 \models \forall a_1:A_1, \ldots, \forall a_n:A_n, \varphi$.  Then $\lift(\tau)$ is the term of type $\Sigma \{ x:X \mid \varphi \}$ sending any tuple $(\alpha_i) \in \prod_{i=1}^n \Hom(U, A_i)$ to the unique factorization of $\tau(\alpha_1, \ldots, \alpha_n)$ through $\Hom(U, \Sigma \{ x:X \mid \phi \})$.
	\end{definition}
	...
	\begin{definition}
		Suppose $\varphi$ is a term of type $\Omega$ with free variables $x:X, a_1:A_1, \ldots, a_n:A_n$, and suppose that $1 \models \forall a_1:A_1, \ldots, \forall a_n:A_n, \exists! x:X, \varphi$.  Then $\iota x:X \mid \varphi$ (to be understood informally as ``the unique $x$ of type $X$ such that $\varphi$'') is the term of type $X$ with free variables $a_1:A_1, \ldots, a_n:A_n$ which sends a tuple $(\alpha_i) \in \prod_{i=1}^n \Hom(U, A_i)$ to the unique factorization of $\{ x:X \mid \varphi \}(\alpha_1, \ldots, \alpha_n)$ through the singleton map $\{ \cdot \} : X \to PX$.
	\end{definition}
	...
	\begin{definition}
		Suppose we have a term $\varphi$ of type $Y^X$ and a term $\tau$ of type $X$, both with the same set of free variables $a_1:A_1, \ldots, a_n:A_n$.  Then $\varphi(\tau)$ is the term of type $Y$ with the same free variables defined by $\iota y:Y \mid (\tau, y) \in \drop(\varphi)$. 
	\end{definition}
		
	\begin{definition}
		Suppose we have a term $\tau$ of type $Y$ with free variables $x:X, a_1:A_1, \ldots, a_n:A_n$.  Then $\lambda x:X \,.\, \tau$ is the term of type $Y^X$ with free variables $a_1:A_1, \ldots, a_n:A_n$ defined by $\lift(\{ p : X\times Y \mid \exists! x:X, p = (x, \tau) \})$, where $p$ is a fresh free variable not in the set $x, a_1, \ldots, a_n$.
	\end{definition}
	...
	\begin{corollary}
		$U \models (\exists x:X, \varphi)[a_1:=\alpha_1, \ldots, a_n:=\alpha_n]$ if and only if there exists an epimorphism $\pi : V \to U$ and a generalized element $x \in Hom(V, X)$ such that $V \models \varphi[x:=x, a_1:=\alpha_1\circ\pi, \ldots, a_n:=\alpha_n\circ\pi]$.
	\end{corollary}
	...
	\begin{corollary}
		$U \models \bot[a_1 := \alpha_1, \ldots, a_n := \alpha_n]$ if and only if $U$ is an initial object.
	\end{corollary}
	...
	\begin{proposition}
		For any object $Y$ of a topos, $\PartialHom({-}, Y)$ is representable by an object $\tilde Y := \Sigma\{ S : PY \mid \forall y_1, y_2:Y, (y_1\in S \land y_2\in S) \rightarrow y_1 = y_2 \}$ called the partial morphism classifier of $Y$, with a given monomorphism $Y \hookrightarrow \tilde Y$ corresponding to $\lambda y:Y \, . \, \lift(\{ y \})$, such that $(Y, \id_Y) : \tilde Y \dashrightarrow Y$ is the universal partial morphism into $Y$.
	...
	\begin{proposition}
		Let $D$ be an object of a topos $\calT$.  Then $\calT / D$ is also a topos, with finite products given as above, and with power objects $P_{\calT / D}(X) \simeq \Sigma \{ p : D \times P_{\calT}(X) \mid \forall x:X, x \in \pi_2(p) \rightarrow \deg(x) = \pi_1(p) \}$.
		...
		\begin{proof}
			...
			Now, suppose we have two objects $X, Y$ of $\calT / D$ and a subobject $S$ of $X \times_D Y$.  We then define the morphism $X \to P_{\calT / D}(Y)$ to correspond to the term $\lambda x : X \, . \, \lift((\deg(x), \{ y:Y \mid \exists! s:S, \pi_1(s) = x \land \pi_2(s) = y \}))$.  Conversely, given a morphism $f : X \to P_{\calT / D}(Y)$ in $\calT / D$, we can define the corresponding subobject of $X \times_D Y$ as $\Sigma \{ p : X \times_D Y \mid \pi_2(p) \in \pi_2(\drop(f(\pi_1(p)))) \}$.  Here, in the last part, if $p : X \times_D Y$, then $\pi_1(p) : X$, $f(\pi_1(p)) : P_{\calT / D}(Y)$, $\drop(f(\pi_1(p))) : D \times P_{\calT}(Y)$, and $\pi_2(\drop(f(\pi_1(p)))) : P_{\calT}(Y)$, while $\pi_2(p) : Y$.  It now remains to check that these give inverse morphisms of functors, which should be straightforward if long to do using the previously developed Kripke-Joyal semantics.
		\end{proof}
	\end{proposition}
	
	In particular, since the final object of $\calT / D$ is $\id_D : D \to D$, we get that the subobject classifier object of $\calT / D$ is $\Sigma \{ p : D \times P_{\calT}(D) \mid \forall d:D, d \in \pi_2(p) \rightarrow d = \pi_1(p) \}$, with $\mathsf{true}_{\calT / D} : D \to \Omega_{\calT / D}$ corresponding to $\lambda d : D \, . \, \lift((d, \{ d \}))$.  However, intuitively speaking, the condition implies that $\pi_2(p)$ is a subset of the singleton $\{ \pi_1(p) \}$, and that subset is equivalent to the generalized element of $\Omega$ given by $\pi_1(p) \in \pi_2(p)$.  It is straightforward to formalize this argument to show:
	
	\begin{corollary}
		The subobject classifier object of $\calT / D$ is $\Omega_{\calT / D} \simeq D \times \Omega_{\calT}$, with $\mathsf{true}_{\calT / D} : D \to \Omega_{\calT / D}$ being $(\id_D, \mathsf{true}_D)$.  Given a subobject $X'$ of $X$ in $\calT / D$, the corresponding morphism $X \to \Omega_{\calT / D}$ is $(\deg_X, \chi_{X'})$ where $\chi_{X'} : X \to \Omega_{\calT}$ is the characteristic morphism taken in $\calT$.
	\end{corollary}
	
	Now, if we follow through the general construction of exponentials in a topos, we get that $(X \multimap Y)_{\calT / D} \simeq \Sigma\{ p : D \times P(X \times_D Y) \mid (\forall q : X \times_D Y, q \in \pi_2(p) \rightarrow \deg(q) = \pi_1(p)) \land (\forall x:X, \deg(x) = \pi_1(p) \rightarrow \exists! q : X \times_D Y, q \in \pi_2(p) \land \pi_1(q) = x) \}$.  Here, the first condition essentially says that every pair in the subset has the correct degree, and the second condition says that for every $x$ with correct degree, there exists a unique pair in the subset with $x$ as its first coordinate.  The second condition then allows us to construct the evaluation morphism $(X \multimap Y)_{\calT / D} \times_D X \to Y$ as $\lambda p \, . \, \pi_2(\iota q \mid q \in \pi_2(\drop(\pi_1(p))) \land \pi_1(q) = \pi_2(p))$.  Given a $D$-homogeneous morphism $f : Z \times_D X \to Y$, the corresponding morphism $Z \to (X \multimap Y)_{\calT / D}$ corresponds to $\lambda z : Z \, . \, \lift((\deg(z), \{ p : X \times_D Y \mid \exists! q : Z \times_D X, \pi_1(q) = z \land \pi_2(q) = \pi_1(p) \land f(q) = \pi_2(p) \}))$.
	...
	\begin{proposition}
		...
		$$(X \multimap Y)_{\calT / D} \simeq \Sigma\{ p : (X \multimap \tilde Y)_{\calT} \times D \mid \widetilde{\deg_Y} \circ \pi_1(p) = \delta(\pi_2(p)) \circ \deg_X \}.$$
	...
	\begin{example}
		If $X$ is a topological space, then we can define a Lawvere-Tierney topology where $j_U : \Omega(U) \to \Omega(U)$ sends an ideal $\mathcal{F}$ of the open subsets of $U$ to the collection of $V \subseteq U$ such that there exists an open cover $\{ W_i \mid i \in I \}$ of $V$ such that each $W_i$ is in $\mathcal{F}$.
	\end{example}
	...
	\begin{proposition}
		The subobject classifier of $\Sh(j)$ is $\Omega_{\Sh(j)} \simeq \Sigma \{ p : \Omega_{\calT} \mid j(p) \rightarrow p \}$.
	\end{proposition}
	
	For exponentials in the cartesian closed category $\Sh(j)$, we have:
	
	\begin{proposition}
		If $Y$ is a $j$-sheaf and $X$ is any object, then $(X \multimap Y)_{\calT}$ is also a $j$-sheaf.  If in addition $X$ is a $j$-sheaf, then $(X \multimap Y)_{\Sh(j)} \simeq (X \multimap Y)_{\calT}$.
	...
	\begin{proposition}
		For any object $Y$ of $\Sh(j)$, $(\tilde Y)_{\Sh(j)} \simeq \Sigma \{ \tilde y : (\tilde Y)_{\calT} \mid j(\mathsf{def}(\tilde y)) \rightarrow \mathsf{def}(\tilde y) \}$.
	\end{proposition}
	...
	\begin{itemize}
		\item Type forming constructs: $\times$, $\Omega$, $1$, $0$, $\multimap$, $\Sigma$, $\sqcup$, $/$.
		\item Term forming constructs: $=$, $\in$, $\land$, $\lor$, $\lnot$, $\rightarrow$, $\top$, $\bot$, $\forall$, $\exists$, $\exists!$, $\lambda$, function application, subset comprehension $\{ x : X \mid \cdots \}$, $\iota$, $\mathsf{drop}$, $\mathsf{lift}$.
	\end{itemize}
	...
	\begin{itemize}
		...
		\item $\varphi \mathrm{~true}$, indicating that the term $\varphi$ (where implicitly we suppose $\varphi : \Omega$ is assumed or provable) is considered to be a true proposition.
	\end{itemize}
	...
	\begin{itemize}
		\item ($\beta$ rule for subsets) $\forall t_1 : T_1, \ldots, \forall t_n : T_n, \forall x : X, x \in \{ y : X \mid \varphi \} \leftrightarrow \varphi [ y := x ]$ (where the last term is a synactic substitution of $x$ in place of $y$ which roughly will be defined recursively the way that you would expect, and the leading $\forall$ terms are for the free variables in $\tau$ other than $y$).
		...
		\item (propositional extensionality) $\forall p, q : \Omega, ((p \rightarrow q) \land (q \rightarrow p)) \rightarrow p = q$.
		...
		\item $\forall y : X, \varphi [x := y] \rightarrow \exists! \tilde y : \{ x : X \mid \varphi \}, \drop(\tilde y) = y$.
		\item If $\varphi$ can be proven to satisfy the appropriate unique existence condition, then $\forall t_1 : T_1 \ldots t_n : T_n, \varphi[x := (\iota x : X \mid \varphi)]$.
	\end{itemize}
